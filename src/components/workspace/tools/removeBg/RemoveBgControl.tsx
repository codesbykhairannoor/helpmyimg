...
// Kontrol UI untuk Background Removal (Tunggal & Batch) dengan Mode Adaptif & Reset/Upload Terpisah
...
export const RemoveBgControl: React.FC<RemoveBgControlProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Mode Selection UI (Adaptive Auto-Detect + Manual Overrides) */}
      {setImageType && (
        <div className="space-y-3 bg-dark-800/50 p-4 rounded-2xl border border-dark-600/80 shadow-inner">
          <label className="text-xs font-bold text-white flex items-center justify-between">
            ...
            <span className="text-[10px] text-neon-cyan font-mono font-semibold px-2 py-0.5 rounded-full bg-neon-cyan/10 border border-neon-cyan/30">
              {imageType === 'auto'
                ? t('remove.modeAutoActive', { defaultValue: 'Adaptive Auto' })
                : imageType === 'logo'
          ...
          <div className="grid grid-cols-3 gap-1.5 bg-dark-900/70 p-1.5 rounded-xl border border-dark-700">
            {/* 1. Auto Adaptive (Default) */}
            <button
              type="button"
            ...
            >
              <Sparkles className={`w-4 h-4 ${imageType === 'auto' ? 'text-neon-cyan' : 'opacity-70'}`} />
              <span className="text-[11px] font-bold truncate">{t('remove.modeAuto', { defaultValue: '✨ Auto' })}</span>
              <span className="text-[8px] text-slate-500 font-medium text-center truncate">{t('remove.modeAutoDesc', { defaultValue: 'Adaptive' })}</span>
            </button>
            ...
            >
              ...
              <span className="text-[11px] font-bold truncate">{t('remove.modeAi', { defaultValue: 'Photo AI' })}</span>
              <span className="text-[8px] text-slate-500 font-medium text-center truncate">{t('remove.modeAiDesc', { defaultValue: 'Human/Pet' })}</span>
            </button>
            ...
            >
              ...
              <span className="text-[11px] font-bold truncate">{t('remove.modeLogo', { defaultValue: 'Logo' })}</span>
              <span className="text-[8px] text-slate-500 font-medium text-center truncate">{t('remove.modeLogoDesc', { defaultValue: 'Vector 0ms' })}</span>
            </button>
          </div>

          {/* Smart Pill: Auto Detected Image Type */}
          {imageType === 'auto' && (
            <div className="flex items-center justify-between px-3 py-1.5 rounded-lg bg-dark-900/80 border border-dark-700 text-[11px]">
              <span className="text-slate-400 flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-neon-cyan" />
                <span>{t('remove.detectedType', { defaultValue: 'Auto Detection:' })}</span>
              </span>
              <span className="font-bold text-neon-cyan font-mono">
                {detectedType === 'logo'
                  ? t('remove.detectedLogo', { defaultValue: '🏷️ Logo / Vector Graphic (0ms)' })
                  : t('remove.detectedPhoto', { defaultValue: '📷 Natural Photo / Portrait (AI)' })}
              </span>
            </div>
          )}
        </div>

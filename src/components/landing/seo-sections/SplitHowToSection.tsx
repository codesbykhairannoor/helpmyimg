...
export const SplitHowToSection: React.FC<SectionProps> = ({ section }) => (
  <section className="seo-section split-how-to" style={{ padding: '60px 24px', margin: '40px 0' }}>
    <h2 style={{ fontSize: 'clamp(1.35rem, 4vw, 2.5rem)', fontWeight: 800, textAlign: 'left', marginBottom: 40, color: 'var(--text-main)', paddingBottom: 16, borderBottom: '2px solid var(--border-color)' }}>{section.title}</h2>
      ...
      {section.steps?.map((step, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 24, padding: 32, background: 'var(--bg-card)', borderRadius: 24, border: '1px solid var(--border-color)' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 'clamp(0.95rem, 3vw, 1.15rem)', color: 'var(--text-muted)', lineHeight: 1.6 }}>{step.description}</p>
          </div>
        </div>
      ))}
    </div>

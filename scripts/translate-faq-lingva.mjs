...
async function translateText(text, targetLang) {
  try {
    const res = await lingva(text, 'en', targetLang);
    return res.text;
  } catch (err) {
...
async function run() {
  for (const lang of languages) {
    ...
    // Check if FAQ is already translated
    if (data['landing.global.faq.title'] && data['landing.global.faq.title'] !== faqKeys['landing.global.faq.title']) {
       console.log(`[${lang}] FAQ already translated, skipping.`);
       continue;
    }

...
const missingKeys = {
  ...
  "home.geo.quote.author": "Dr. Sarah Chen",
  "home.geo.quote.role": "Lead Web Security Researcher, DataPrivacy Institute",
...
const originalStrings = keysList.map(k => missingKeys[k]);

// We will batch strings to avoid massive payloads that might break Lingva
const BATCH_SIZE = 10; 
const SEPARATOR = ' ||| ';
...
async function translateText(text, targetLang) {
  try {
    ...
    const res = await lingva(text, 'en', tLang);
    return res.text;
  } catch (err) {
    throw new Error(`Lingva failed: ${err.message}`);
  }
}
...
async function run() {
  for (const lang of languages) {
    ...
    console.log(`\n[${lang}] Translating via Lingva...`);
    
    // Process in batches
    for (let i = 0; i < keysList.length; i += BATCH_SIZE) {
      const batchKeys = keysList.slice(i, i + BATCH_SIZE);
      const batchStrings = originalStrings.slice(i, i + BATCH_SIZE);
      const combinedText = batchStrings.join(SEPARATOR);
      ...
      try {
        ...
        if (translatedStrings.length !== batchStrings.length) {
          console.error(`  [${lang}] Length mismatch in batch! Falling back to 1-by-1.`);
          for (let j = 0; j < batchKeys.length; j++) {
             await new Promise(r => setTimeout(r, 1000));
             try {
                const indRes = await translateText(batchStrings[j], lang);
                setNestedValue(langData, batchKeys[j], indRes.trim() || batchStrings[j]);
                updated = true;
             } catch(e) {
                console.error(`  [${lang}] Failed single: ${batchKeys[j]}`);
             }
          }
        } else {
          for (let j = 0; j < batchKeys.length; j++) {
            setNestedValue(langData, batchKeys[j], translatedStrings[j] || batchStrings[j]);
            updated = true;
          }
          console.log(`  [${lang}] Batch ${i/BATCH_SIZE + 1} complete.`);
        }
      } catch (e) {
        console.error(`  [${lang}] Batch Error:`, e.message);
      }
    }
